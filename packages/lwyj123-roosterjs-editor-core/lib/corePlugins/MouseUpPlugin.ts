...
export default class MouseUpPlugin implements EditorPlugin {
    ...
    onPluginEvent(event: PluginEvent) {
        if (
            ...
            this.editor
                .getEditorHost()
                .addEventListener('mouseup', this.onMouseUp as EventListener, true /*setCapture*/);
    ...
    private removeMouseUpEventListener() {
        if (this.editor && this.mouseUpEventListerAdded) {
            ...
            this.editor.getEditorHost().removeEventListener('mouseup', this.onMouseUp as EventListener, true);
        }
    }

    private onMouseUp = (rawEvent: MouseEvent): void => {
        if (this.editor) {
            this.removeMouseUpEventListener();

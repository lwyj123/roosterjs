import { arrayPush, Browser, isCharacterValue } from 'lwyj123-roosterjs-editor-dom';
import {
    ChangeSource,
    ContextMenuProvider,
    DOMEventHandler,
...
export default class DOMEventPlugin implements PluginWithState<DOMEventPluginState> {
    ...
    initialize(editor: IEditor) {
        ...
        // 7. onBlur handlers
        if (Browser.isSafari) {
            host.addEventListener('mousedown', this.onMouseDownDocument as EventListener, true /*useCapture*/);
            host.addEventListener('keydown', this.onKeyDownDocument as EventListener);
    ...
    dispose() {
        ...
        if (host && Browser.isSafari) {
            host.removeEventListener('mousedown', this.onMouseDownDocument as EventListener, true /*useCapture*/);
            host.removeEventListener('keydown', this.onKeyDownDocument as EventListener);

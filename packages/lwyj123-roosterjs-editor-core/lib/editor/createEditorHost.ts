import { Browser, safeInstanceOf } from 'lwyj123-roosterjs-editor-dom';
import { EditorHost } from 'lwyj123-roosterjs-editor-types';
...
abstract class EditorHostBase<T extends ShadowRoot | Document> implements EditorHost {
    ...
    constructor(
        ...
        if (!this.document.defaultView) {
            throw new Error('document default view is not found');
        }
...
class ChromeEditorHost extends EditorHostBase<GetSelection & (ShadowRoot | Document)> {
    ...
    getSelection(): Selection {
        const selection = this.shadowRootOrDoc.getSelection();
        if (!selection) {
            throw new Error('selection not found in Chrome');
        }
        return selection;
    }
}
...
class FirefoxEditorHost extends EditorHostBase<ShadowRoot | Document> {
    getSelection(): Selection {
        ...
        if (!selection) {
            throw new Error('selection not found in Firefox');
        }
        return selection;
    }
}
...
class ShadowSelection implements Selection {
    ...
    modify(
        ...
        throw new Error('Method "modify" not implemented.');
    }
    toString(): string {

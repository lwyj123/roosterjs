import * as roosterEditorApi from 'lwyj123-roosterjs-editor-api';
import * as roosterEditorDom from 'lwyj123-roosterjs-editor-dom';
import * as TestHelper from 'lwyj123-roosterjs-editor-api/test/TestHelper';
import { DocumentCommand, FontSizeChange, IEditor } from 'lwyj123-roosterjs-editor-types';
...
parameters.forEach(({ description, key, command }) => {
    it(description, () => {
        ...
        const shortCutFeature = ShortcutFeatures.defaultShortcut;
        const spya = spyOn(editor.getEditorHost(), 'execCommand');
        shortCutFeature.handleEvent(event, editor);

...
import { contains, getTagOfNode, safeInstanceOf, VTable } from 'lwyj123-roosterjs-editor-dom';
import { getCellAtCursor } from '../utils/getCellAtCursor';
import { getCellCoordinates } from '../utils/getCellCoordinates';
import { getTableAtCursor } from '../utils/getTableAtCursor';
import { IEditor, PluginMouseDownEvent } from 'lwyj123-roosterjs-editor-types';
...
export function handleMouseDownEvent(
    ...
    if (which == LEFT_CLICK && !shiftKey) {
        ...
        if (getTableAtCursor(editor, event.rawEvent.target)) {
            ...
            const mouseMoveListener = onMouseMove(state, editor) as EventListener;
